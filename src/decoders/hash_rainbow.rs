    ...
    let table = match algorithm {
        HashAlgorithm::Md5 => {
            if tables.md5.is_none() {
                tables.md5 = Some(build_table(&COMMON_PASSWORDS, |w| hex_digest_md5(w)));
            }
            tables.md5.as_ref()
        }
        HashAlgorithm::Sha1 => {
            if tables.sha1.is_none() {
                tables.sha1 = Some(build_table(&COMMON_PASSWORDS, |w| hex_digest_sha1(w)));
            }
            tables.sha1.as_ref()
        }
        HashAlgorithm::Sha256 => {
            if tables.sha256.is_none() {
                tables.sha256 = Some(build_table(&COMMON_PASSWORDS, |w| hex_digest_sha256(w)));
            }
            tables.sha256.as_ref()
        }
        HashAlgorithm::Sha512 => {
            if tables.sha512.is_none() {
                tables.sha512 = Some(build_table(&COMMON_PASSWORDS, |w| hex_digest_sha512(w)));
            }
